...
mat camport_device_impl::next_depth_frame() {
  ...
  LOGD("Re-enqueue buffer(%p, %d)", dev_data_->frame_data.userBuffer,
       dev_data_->frame_data.bufferSize);
  CHECK_OK(TYEnqueueBuffer(dev_data_->hDevice, dev_data_->frame_data.userBuffer,
                           dev_data_->frame_data.bufferSize));
  return m;
}

std::string camport_device_impl::id() { return dev_data_->base_info.id; }
