...
void camport_wrapper_impl::init_device(const TY_DEVICE_BASE_INFO &selectedDev) {
  ...
  if (allComps & TY_COMPONENT_DEPTH_CAM) {
    ...
    for (size_t idx = 0; idx < image_mode_list.size(); idx++) {
      ...
      cout << "depth image width:" << dev_data_p->width
           << " height:" << dev_data_p->height << endl;
      if (TYImageWidth(entry.value) == 640 ||
          TYImageHeight(entry.value) == 640) {
        LOGD("Select Depth Image Mode: %s", entry.description);
        int err = TYSetEnum(hDevice, TY_COMPONENT_DEPTH_CAM, TY_ENUM_IMAGE_MODE,
                            entry.value);
  ...
  LOGD("     - Enqueue buffer (%p, %d)", dev_data_p->frameBuffer[0].data(),
       frameSize);
  CHECK_OK(
      TYEnqueueBuffer(hDevice, dev_data_p->frameBuffer[0].data(), frameSize));
  LOGD("     - Enqueue buffer (%p, %d)", dev_data_p->frameBuffer[1].data(),
       frameSize);
  CHECK_OK(
      TYEnqueueBuffer(hDevice, dev_data_p->frameBuffer[1].data(), frameSize));
  ...
  if (hasTrigger) {
    ...
    CHECK_OK(TYSetStruct(hDevice, TY_COMPONENT_DEVICE, TY_STRUCT_TRIGGER_PARAM,
...
std::shared_ptr<camport_wrapper> get_wrapper() {
  static std::shared_ptr<camport_wrapper> wrapper =
      std::make_shared<camport_wrapper_impl>();
  return wrapper;
}
/////////////////////////
